    ...
    namespace V2 {
        ...
        public class EmployeeRepository : SqlRepository, IEmployeeRepository {
            ...
            public async Task<Employee> GetEmployeeByCchIdAsync(int cchId) {
                ...
                using (var ctx = new EmployeeContext(_connectionString)) {
                    employee = await ctx.Employees.FirstOrDefaultAsync(p => p.CchId.Equals(cchId) && p.CurrentRecordInd == true);
                }
            ...
            public async Task<Member> GetMemberByCchIdAsync(int cchId)
            {
                ...
                using (var ctx = new EmployeeContext(_connectionString))
                {
                    member = await ctx.Members.FirstOrDefaultAsync(p => p.CchId.Equals(cchId) && p.CurrentRecordInd == true);
                }
                ...
                using (var ctx = new EmployeeContext(_connectionString)) {
                    var employee = await ctx.Employees
                        .FirstOrDefaultAsync(
                            ...
                            && p.CurrentRecordInd.Equals(true));
